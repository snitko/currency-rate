...
RSpec.describe CurrencyRate::Synchronizer do
  ...
  describe "#sync!" do
    before do
      ...
      allow(@synchronizer).to receive(:adapters).with("crypto").and_return(@crypto_adapters)
      ...
      allow(@synchronizer).to receive(:adapters).with("fiat").and_return(@fiat_adapters)
    end
